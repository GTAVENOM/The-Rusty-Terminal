...
pub enum Event {
    ...
    /// RustyTerminal patch: a shell-integration OSC sequence (OSC 133
    /// prompt marks, OSC 9;9 / OSC 7 cwd reports) observed in the PTY
    /// output stream. Payload is the raw OSC body, e.g. `133;D;0`.
    ShellIntegrationOsc(String),
}
...
impl Debug for Event {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Event::ShellIntegrationOsc(body) => {
                write!(f, "ShellIntegrationOsc({body})")
            },
        }
    }
}
