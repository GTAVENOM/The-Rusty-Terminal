    ...
    U: EventListener + Send + 'static,
{
        ...
        X: Write,
    {
        ...
        loop {
            ...
            // RustyTerminal patch: passively scan for shell-integration OSC
            // marks before the VTE parser consumes the bytes.
            for osc_body in state.osc_scanner.scan(&buf[..unprocessed]) {
                self.event_proxy.send_event(Event::ShellIntegrationOsc(osc_body));
            }
...
pub struct State {
    ...
    /// RustyTerminal patch: stateful scanner for shell-integration OSC
    /// sequences in the raw output stream.
    osc_scanner: OscScanner,
}

/// RustyTerminal patch: incremental scanner that recognizes shell-integration
/// OSC sequences (`OSC 133;...`, `OSC 9;9;...`, `OSC 7;...`) in the PTY
/// output stream, robust to sequences split across read chunks. The bytes are
/// still fed to the VTE parser unchanged; this is a passive observer.
#[derive(Default)]
...
enum OscScanState {
    ...
    Ground,
    Esc,       // saw ESC
    Collect,   // inside OSC body
    CollectEsc, // inside OSC body, saw ESC (possible ST terminator)
}

impl OscScanner {
    const MAX_BODY: usize = 1024;

    /// Scan a chunk of PTY output, returning the bodies of any complete
    /// shell-integration OSC sequences found.
    pub fn scan(&mut self, bytes: &[u8]) -> Vec<String> {
        ...
        for &b in bytes {
            match self.state {
                ...
                OscScanState::Esc => {
                    if b == b']' {
                        self.state = OscScanState::Collect;
                        self.body.clear();
                    } else if b == 0x1b {
                        // stay in Esc
                    } else {
                ...
                OscScanState::Collect => {
                    if b == 0x07 {
                        // BEL terminator
                        self.finish(&mut found);
                    } else if b == 0x1b {
                        self.state = OscScanState::CollectEsc;
                    } else if self.body.len() < Self::MAX_BODY {
                        self.body.push(b);
                    } else {
                        // Oversized OSC (e.g. clipboard payload) — not ours.
                        self.state = OscScanState::Ground;
                        self.body.clear();
                    }
                },
                OscScanState::CollectEsc => {
                    if b == b'\\' {
                        // ST terminator (ESC \)
                        self.finish(&mut found);
                    } else {
                        // Not a terminator; abandon (nested ESC in OSC is
                        // invalid anyway).
                        self.state = OscScanState::Ground;
                        self.body.clear();
                    }
    ...
    fn finish(&mut self, found: &mut Vec<String>) {
        self.state = OscScanState::Ground;
        let body = std::mem::take(&mut self.body);
        let interesting = body.starts_with(b"133;")
            || body.starts_with(b"9;9;")
            || body.starts_with(b"7;");
        if interesting {
